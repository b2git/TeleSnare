...
##############################
# BusinessLogic：解析 HTTP 响应，通过 JSON 或正则匹配提取凭证
##############################
class BusinessLogic:
    def __init__(self, config):
        try:
    ...
    def extract_credentials(self, response_str):
        ...
        if response_str.lstrip().startswith("{") or response_str.lstrip().startswith("["):
            try:
                ...
                tokens.extend(jt)
                chatids.extend(jc)
                debug("JSON解析成功，找到 tokens: " + str(jt) + ", chatids: " + str(jc))
            except Exception as e:
                debug("JSON解析失败，继续正则匹配: " + str(e))
        else:
        ...
        try:
            ...
            tokens.extend(rt)
            chatids.extend(rc)
            debug("正则匹配到 tokens: " + str(rt) + ", chatids: " + str(rc))
        except Exception as e:
            debug("正则匹配异常: " + str(e))
        # 去重处理
...
##############################
# UIManager：提供完整界面，包括工具栏、日志、结果表格和导出 CSV 功能
##############################
class UIManager:
    def __init__(self):
        try:
    ...
    def export_csv(self, event):
        try:
            ...
            # 对于 CSV 写入，使用二进制模式，确保在 Windows 上正常工作
